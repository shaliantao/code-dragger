...
import { Event, Emitter, Relay, EventMultiplexer } from '@base/common/event';
import {
  IDisposable,
  toDisposable,
  combinedDisposable,
...
export interface IChannelSenderOptions extends IBaseChannelOptions {
  ...
  /**
   * If provided, will run after got the result.
   */
  onResult?: (result: unknown) => void;
  /**
   * If provided, will run on error.
   */
  onError?: (error: unknown, propKey: string) => void;
}
...
export function logWithColors(
  ...
  let args = [
    `%c[${direction}]%c[${String(totalLength).padStart(7, ' ')}]%c[len: ${String(
      msgLength,
    ).padStart(5, ' ')}]%c${String(req).padStart(5, ' ')} - ${str}`,
    'color: darkgreen',
    'color: grey',
    'color: grey',
    `color: ${color}`,
  ...
  console.log(...(args as [string, ...string[]]));
}
