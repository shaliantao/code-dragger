...
import { ICodeWindow } from '@base/windows/windows';
import { handleVetos } from '@base/lifecycle/lifecycle';
...
export class LifecycleMainService extends Disposable implements ILifecycleMainService {
  ...
  private onBeforeUnloadWindowInRenderer(
    ...
    return new Promise<boolean>((resolve) => {
      ...
      const okChannel = `smart:ok${oneTimeEventToken}`;
      const cancelChannel = `smart:cancel${oneTimeEventToken}`;
      ...
      window.send('smart:onBeforeUnload', { okChannel, cancelChannel, reason });
    });
  }
  ...
  private onWillUnloadWindowInRenderer(window: ICodeWindow, reason: UnloadReason): Promise<void> {
    return new Promise<void>((resolve) => {
      const oneTimeEventToken = this.oneTimeListenerTokenGenerator++;
      const replyChannel = `smart:reply${oneTimeEventToken}`;

      ipc.once(replyChannel, () => resolve());

      window.send('smart:onWillUnload', { replyChannel, reason });
    });
  }
