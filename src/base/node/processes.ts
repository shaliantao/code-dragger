...
} from '@base/common/processes';
import * as Types from '@base/common/types';
import { LineDecoder } from '@base/node/decoder';
  ...
  if (Platform.isWindows) {
    try {
      ...
      return new Promise((resolve) => {
        ...
        killProcess.once('exit', (code, _signal) => {
          if (code === 0) {
      ...
      return Promise.resolve({
        success: false,
        error: err,
        code: err.status ? getWindowsCode(err.status) : TerminateResponseCode.Unknown,
      });
    }
  } else if (Platform.isLinux || Platform.isMacintosh) {
    try {
      const cmd = FileAccess.asFileUri('@base/node/terminateProcess.sh', require).fsPath;
      return new Promise((resolve) => {
        cp.execFile(
          cmd,
          [process.pid!.toString()],
          { encoding: 'utf8', shell: true } as cp.ExecFileOptions,
          (err, _stdout, _stderr) => {
            if (err) {
              resolve({ success: false, error: err });
            } else {
...
export abstract class AbstractProcess<TProgressData> {
  ...
  private useExec(): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      ...
      cmdShell.on('error', (_error: Error) => {
        return resolve(true);
      });
      cmdShell.on('exit', (_data: any) => {
        return resolve(false);
      });
    });
  }
